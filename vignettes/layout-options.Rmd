...
shinyGovstyle provides a set of layout functions that produce the HTML structure GOV.UK Frontend CSS expects. This vignette covers every layout function available and explains how they fit together to build a complete app.
...
Creates the black GOV.UK header bar containing your department logo and service name.
...
Displays a phase banner immediately below the header, used to indicate the maturity of your service (e.g. Alpha or Beta).
...
Creates the GOV.UK footer. Use `full = TRUE` to include the OGL licence logo and Crown copyright statement. You can add support links that point either to internal hidden tab panels or to external URLs.

```{r, eval = FALSE}
# Minimal footer
footer()

# Full footer with support links
footer(
  full = TRUE,
  links = c(
...
A thin wrapper that produces a `<p class="govuk-body">` paragraph element. Use it when you want to add body text with correct GOV.UK styling without writing raw tag calls.
...
Use a hidden `tabsetPanel` for the content area, and `observeEvent()` in your server to switch between panels when a navigation link is clicked. Call `update_service_navigation()` alongside `updateTabsetPanel()` to keep the active state of the nav bar in sync. The excerpts below show the structural pattern — replace `"Content"` with your actual UI:

```{r, eval = FALSE}
# ui.R — excerpt showing tab panel structure
shiny::tabsetPanel(
  type = "hidden",
  id = "main_panels",
...
shiny::observeEvent(input$nav_summary, {
  ...
  shinyGovstyle::update_service_navigation(session, "nav_summary")
})
```

Repeat the `observeEvent` block for each navigation link. `update_service_navigation()` is also useful when navigating between pages via buttons or back links that exist within the content rather than in the navigation bar itself.
