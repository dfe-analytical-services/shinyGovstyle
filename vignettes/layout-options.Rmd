...
- **Page-level components** (`header()`, `footer()`, `phase_banner()`, `cookieBanner()`, `skip_to_main()`, and `service_navigation()`) form the frame of the page that sits outside the main content area.
- **Content layout functions**: `gov_main_layout()` holds the main content area, and `gov_row()` and `gov_box()` add grid columns inside it when you need them.

---

## `gov_page()`

`gov_page()` is a small wrapper around [bslib](https://rstudio.github.io/bslib/)'s `page_fluid()`. `bslib` is now generally the recommended way to build R Shiny UIs, in place of `shiny::fluidPage()`, and other `bslib` components can be mixed freely into a shinyGovstyle app. Use `gov_page()` as the very first thing your UI creates: everything else in this vignette goes inside it.
...
- **`width`** (see [Page width](#page-width) below): sets a default width for every shinyGovstyle component used inside it, so you don't have to repeat `width = ` on each one individually.
...
### Content width vs page width {#content-width}

`gov_page(width = ...)` only sets the *outer* ceiling: how wide the page frame is allowed to get. It's a separate thing from how wide any one piece of *content* looks, which you control on the content itself.

**Recommended setup:** for a dashboard-style app, set `gov_page(width = "full")` so wide tables, charts and value boxes get the room they need. Then, for any text-heavy page or section within that same app (a cookies page, a user guide, an accessibility statement), wrap its content in a `div` with GOV.UK's `govuk-!-width-two-thirds` class, rather than letting it stretch the full width of the page. The class works on its own, without the grid, and goes back to full width on small screens.

> **Avoid full-width body text.** GOV.UK's own [Design System guidance](https://design-system.service.gov.uk/styles/layout/) recommends keeping body text to a two-thirds width even on a wide page, so lines don't get so long they're hard to read (aim for no more than about 75 characters per line). Paragraphs on a `"full"`-width page with no width set produce lines that stretch the entire screen: technically valid, but noticeably harder to read. Reserve full width for content that actually benefits from the extra room, like a wide table.

In practice this means: pick a page `width` for the *space you need* (e.g. `"full"` for a dashboard with wide tables), then narrow the sections that need it, independently of the page:
...
gov_page(
  ...
  gov_main_layout(
    ...
    govTable(my_wide_data)
  )
)
```

The example showcase app applies the same idea: its Cookies tab sits in a two-thirds column, so it reads as a normal, narrow text page even though the app itself uses `gov_page(width = "full")`.
...
+-------------------------------------------------------+
|  phase_banner()   [optional, e.g. Beta or Alpha]      |
+-------------------------------------------------------+
|                                                       |
...
Content goes straight into `gov_main_layout()`. Items stack one below the other, so a page that is just a heading, some text and a table needs no extra layout functions:

```{r, eval = FALSE}
gov_main_layout(
  heading_text("Page title", size = "l"),
  gov_text("Some introductory text."),
  govTable(my_data)
)
```
...
For a page that is mostly text, use the standard page width and keep the text to two-thirds of it, as the [GOV.UK layout guidance](https://design-system.service.gov.uk/styles/layout/) recommends for readable line lengths. GOV.UK's `govuk-!-width-two-thirds` class does this without the grid, and goes back to full width on small screens:
...
gov_page(
  ...
  gov_main_layout(
    shiny::tags$div(
      class = "govuk-!-width-two-thirds",
      heading_text("Cookies", size = "l"),
      gov_text("This page explains how we use cookies.")
    )
  )
)
```

### Columns with `gov_row()` and `gov_box()`

Reach for the grid only when you need **side-by-side columns**, or a section that should sit in a grid column of a set width. It's a two-function system: `gov_row()` holds the columns, and `gov_box()` is each column.
...
A single full-width box adds nothing over putting the content directly in `gov_main_layout()`, so leave the grid out in that case.
...
## `gov_layout()` — deprecated

`gov_layout()` is deprecated as of shinyGovstyle 0.3.0 and will be removed in shinyGovstyle 1.0.0. Use `gov_main_layout()` with `gov_row()` and `gov_box()` instead; the deprecation warning shows how each argument maps across.
...
Pass a named character vector to `service_navigation()`. The names are displayed as link text; the values become the inputIDs and are used exactly as given, including case. Inside a Shiny module, wrap each value in `ns()` in the module UI, then use the bare ids (e.g. `"nav_summary"`) in the module server with `service_navigation_server()`, `navigate_to()`, or `update_service_navigation()`:
...
`service_navigation()` syncs the browser tab title with the active nav link by default, both for link clicks and for programmatic navigation via `update_service_navigation()`. Pass `page_title_suffix` to get the recommended `"<page> | <service>"` format, or `auto_page_title = FALSE` to opt out. For pages where the heading differs from the nav label (or for pages not in the nav), call `update_page_title()` explicitly. See the [Page titles section of the Headings and text vignette](headings-and-text.html#page-titles) for the full rationale and examples.
