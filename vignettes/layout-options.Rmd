...
shiny::observeEvent(input$next_btn, {
  shinyGovstyle::navigate_to(session, "main_panels", "nav_detail")
})
```

`navigate_to()` assumes the nav link inputId matches the panel value. For mismatched mappings, call `shiny::updateTabsetPanel()` and `update_service_navigation()` separately. See `?navigate_to` and `?update_service_navigation` for full details.
