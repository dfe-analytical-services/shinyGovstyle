...
Sends a message to the browser to update `document.title` — the text
shown in the browser tab. Use this to keep the page title in sync with
the content currently on display.
...
page, modal dialogue links) and the visible page changes without the
user clicking a nav link directly. Also useful when the page heading
differs from the nav link label and you want the tab title to match the
heading.
...
link directly and the page title should just match the link text.
[`service_navigation()`](https://dfe-analytical-services.github.io/shinyGovstyle/reference/service_navigation.md)
syncs the title automatically by default — see its `auto_page_title`
argument.
...
``` r
update_page_title(session, page_title, service_name = NULL)
```
...
- page_title:
  ...
  Character string with the new page title (typically the page heading
  or nav link text)

- service_name:

  Optional character string. When supplied, the browser tab title is set
  to `"<page_title> | <service_name>"` — the GOV.UK recommended format.
  When `NULL`, only `page_title` is shown.
