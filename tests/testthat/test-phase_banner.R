test_that("phase_banner renders type tag and string label", {
  out <- phase_banner("bannerId", "alpha", "Banner test")
...
test_that("label accepts a shiny.tag", {
  out <- phase_banner("bannerId", "beta", shiny::tags$b("Bold label"))
...
test_that("width = 'standard' explicitly still renders the standard class", {
  out <- phase_banner("bannerId", "alpha", "Banner test", width = "standard")
...
test_that("a custom width sets an inline max-width style", {
  out <- phase_banner("bannerId", "alpha", "Banner test", width = "1400px")
  container <- find_tag_required(out, "govuk-width-container")
  expect_identical(
...
test_that("phase_banner errors when both label and feedback_url are given", {
  expect_error(
    phase_banner(
      "bannerId",
      "beta",
...
test_that("phase_banner errors when neither label nor feedback_url are given", {
  expect_error(
    phase_banner("bannerId", "beta"),
