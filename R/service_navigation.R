...
#'
#' @param links A character vector of links to add to the service
#' navigation. Supply a named vector, where names are the link text and values
#' are the inputIDs, to control the inputIDs yourself. Supplied inputIDs are
#' used exactly as given, including case and any `shiny::NS()` namespace
#' prefix, so `c("Summary" = ns("summary"))` works inside a Shiny module. For
#' unnamed elements, the value is used as the link text and an inputID is
#' generated by lowercasing it and replacing every non-alphanumeric character
#' with an underscore, e.g. "Overview page" becomes `overview_page`.
...
#' @param inputId The inputID of the nav link to mark as active. Like other
#'   Shiny update functions, it is namespaced with `session$ns()`, so inside a
#'   module server pass the id without the namespace (e.g. `"summary"` for a
#'   link created with `ns("summary")`).
#'
...
update_service_navigation <- function(
  ...
  # Namespace like other Shiny update functions, so module server code passes
  # the bare id. `fallback` keeps the raw id working when the nav bar lives
  # outside the module, or the caller already applied ns() themselves.
  session$sendCustomMessage(
    "update_service_navigation",
    list(id = session$ns(inputId), fallback = inputId)
  )
}
...
# Named elements are caller-supplied inputIDs and are kept verbatim, so module
# namespaces (e.g. "mod-summary") and mixed case survive. Only unnamed
# elements, where the caller gave link text alone, get a generated inputID.
service_nav_ids <- function(links) {
  ...
  id <- ifelse(
    supplied_id,
    links,
    tolower(gsub("[^a-zA-Z0-9]", "_", links))
  )
  ...
  if (anyDuplicated(id)) {
    stop(
      ...
      ". Supply a named vector to set the inputIDs yourself.",
      call. = FALSE
    )
  }
...
#'
#' @param input_id The inputID for the link, used verbatim.
