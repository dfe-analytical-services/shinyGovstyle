...
- **Content layout functions** (`gov_main_layout()`, `gov_row()`, and `gov_box()`) structure content within the main content area.
...
`gov_page(width = ...)` only sets the *outer* ceiling: how wide the page frame is allowed to get. It's a separate thing from how wide any one piece of *content* looks, which is controlled by `gov_row()`/`gov_box(size = ...)` (see [The primary layout system](#the-primary-layout-system) below).
